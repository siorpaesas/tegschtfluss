<x-layout>
    <x-slot:heading>
        Statistik-Update bearbeite
    </x-slot:heading>
    <p>Kapitu: <strong>{{ $chapter->name }}</strong></p>
    <form action="/chapter/{{$chapter->id}}/stats/edit" method="post">
        @csrf
            ...
            <div class="mt-4 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                <x-form.field>
                    ...
                    <x-form.input
                        ...
                        id="words"
                        placeholder="2222"
                        value="{{ old('words') ? old('words') : $build->words }}"
                        required/>
                ...
                <x-form.field>
                    ...
                    <x-form.input
                        ...
                        id="length"
                        placeholder="9"
                        value="{{ old('length') ? old('length') : $build->length }}"
                        required/>
                ...
                <x-form.field>
                    ...
                    <x-form.input
                        ...
                        name="commit"
                        id="commit"
                        placeholder="Der Durscht uf Ärde besiegt"
                        value="{{ old('commit') ? old('commit') : $build->commit }}"
                    />
                    <x-form.error name="commit"/>
                </x-form.field>
            </div>
            <div class="mt-12 flex items-center gap-x-6">
                <x-form.submit>Stats update</x-form.submit>
